...
func drawHandler(ctx gogui.DrawContext) {
	...
	regPoint := pointsWithDeleting[len(pointsWithDeleting)-1]
	maxX := (1-percent)*(startPoints[len(startPoints)-1].X-regPoint.X) + regPoint.X
	ctx.SetStroke(gogui.Color{0, 0, 1, 1})
	ctx.SetThickness(5)
	ctx.BeginPath()
	deletedPoint := startPoints[len(startPoints)-1]
	...
	if percent < 1 {
		ctx.LineTo(deletedPoint.X, deletedPoint.Y)
	}
	ctx.StrokePath()
	...
	if percent < 1 {
		radius := 5 - 5*percent
		ctx.FillEllipse(gogui.Rect{deletedPoint.X - radius, deletedPoint.Y - radius, radius * 2,
			radius * 2})
	}
}
