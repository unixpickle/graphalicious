...
Animations are essential to beautiful graphs, and a *ChunkView* is responsible for them. It is suggested that *ChunkView*s use [harmonizer](https://github.com/unixpickle/harmonizer) for their animations, since *Harmonizer*s can prevent redundant repainting during simultaneous animations. There are two different kinds of animations: **superficial animations** and **landscape animations**.

A landscape animation is a smooth transition between an old complete landscape and a new complete landscape. Only some changes to the complete landscape (i.e. changes to the *DataSource*) may trigger a landscape animation, while others (i.e. changes in visual style) may not. When a *ChunkView* is performing a landscape animation, we can imagine the complete landscape **morphing** from the old landscape to the new one. The *VisualStyle* interface does not capture said morphing, but instead only represents **final landscapes**. A final landscape is a landscape during which no landscape animations are taking place. While a *VisualStyle* only knows about **final landscapes**, the *ChunkView* has extra information about **morphing landscapes** as well. In particular, while a landscape animation is being performed, a *ChunkView* has two different offsets, widths, and encompassing widths. One set of values is for the final landscape, while the other is for the current morphing landscape. This is useful information because it allows the *ContentView* to know how and where to draw the *ChunkView* at every point during a landscape animation.

A *ChunkView* may perform other animations which do not affect the landscape. These are called superficial animations. For instance, the *ChunkView* might fade in a tooltip or animate colors changing&mdash;anything that does not affect the layout of the landscape.
...
Since the *ChunkView* is responsible for animations, there are some animation-related methods:

 * *void* finishAnimation() - finish the current landscape animation early. This does not apply to superficial animations, which nothing can control directly besides the *ChunkView* itself.
 * [harmonizer.Harmonizer](https://github.com/unixpickle/harmonizer) harmonizer() - get the root *Harmonizer* for this *ChunkView*. When a *ChunkView* needs to be redrawn, it will request the redraw through this *Harmonizer*. If the *ChunkView* has no need for a *Harmonizer*, this may return `null`. Note that, if the *ChunkView* implements any landscape animations, it automatically requires a *Harmonizer* to request redraws.

A *ContentView* should notify a *ChunkView* when the *DataSource* changes. These methods return a boolean value, indicating whether or not a landscape animation has been initiated by the change. These methods also take a boolean *animate* value, suggesting whether or not this change should trigger a landscape animation. The *animate* argument is necessary but not sufficient for a landscape animation to take place.
 ...
 * animationFrame(progress) - tell the *ContentView* the new progress of a running landscape animation. The progress value will be between 0 (just started) and 1 (ending) which indicates how "done" the animation is. After a progress of 1 is reported, the *ChunkView* will emit *animationEnd*. This will not be fired for superficial animations. This event is separate from a paint request; the *ChunkView* will issue a paint request at some point before or after emitting this event.
 * animationEnd() - a landscape animation has ended. This corresponds to a the last animationFrame event for a given animation.
